...
async def serve():
    async with websockets.serve(stream, channel.host, channel.port):
        await asyncio.Future()

if check_port(channel.host, channel.port):
    try:
        log.success("Kanal sunucusu, ws://{}:{} adresinde çalışıyor.".format(channel.host, channel.port))
        asyncio.run(serve())
    ...
    log.failure("Kanal WebSocket sunucusunun portu {} müsait değil.".format(channel.port))
    exit(1)