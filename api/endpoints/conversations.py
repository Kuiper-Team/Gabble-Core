...
async def r_conversations(parameters: data_models.Conversation):
    ...
    try:
        users = generation.aes_decrypt(controls.fetch_from_db("conversations", "uuid", parameters.uuid, column="users")[0], parameters.private_key).split(",")
        users = [item for item in users if item != ""]

        public_key = controls.fetch_from_db("conversations", "uuid", parameters.uuid, column="public_key")[0]
    except Exception:
...
async def conversations_delete(parameters: data_models.Conversation):
    ...
    try:
        data = conversations.delete(parameters.uuid)
    except Exception as code:
        return presets.auto(code)
    else:
